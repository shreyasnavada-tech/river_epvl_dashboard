...
class TDMS_file_handler:
    ...
    # def convert_tdms_to_df(self) -> DataFrame:
    #     """
    #     This method converts tdms into Pandas Dataframe(DF) with index provided by the TDMS file
    #     """
    #     #df = self.tdms_file.as_dataframe()
    #     df = self.tdms_file.as_dataframe(arrow_dtypes=True)
    #     return df

    # def convert_tdms_to_df_with_timestamp(
    #     self, relative_timestamp_as_index: bool = False
    # ) -> DataFrame:
    #     """
    #     This method converts tdms into Pandas Dataframe(DF) with or without timestamps

    #     Args:
    #         relative_timestamp_as_index: False = If True, absolute timestamps are written into DF
    #                                              Else, relative timestamps are written into df
    #     """
    #     df = self.tdms_file.as_dataframe(
    #         time_index=relative_timestamp_as_index,
    #         absolute_time=relative_timestamp_as_index,
    #     )
...
class TRC_Filehandler:
    ...
    def read_csv_to_polars(
        ...
        # 3. Cast and Convert to dataframe with the below column names.
        if self.polars_df is not None:
...
if __name__ == "__main__":
    ...
    # trcfile = TRC_Filehandler(r"C:\Users\shreyasnavada_rideri\Documents\dev\vehicle-automation\motor_dyno_webapp\Data\testTRC_5linestest.trc", dbc_parser)
